...
/datum/configuration
	...
	var/static/vote_autotransfer_limit = 0			// Number of transfer votes allowed
	...
	for(var/t in Lines)
		...
		if(type == "config")
			switch (name)
				...
				if ("vote_autotransfer_limit")
					config.vote_autotransfer_limit = text2num(value)
