	...
	switch(M.a_intent)
		if(I_HELP)
			if(istype(H) && health < config.health_threshold_crit)
				...
				H.visible_message("<span class='danger'>\The [H] performs CPR on \the [src]!</span>", "<span class='danger'>You perform CPR on \the [src]!</span> <span class='warning'>Repeat regularly.</span>")

				adjustOxyLoss(-(min(getOxyLoss(), 5)))
				if (stat == DEAD && bloodstr)
					bloodstr.metabolize(TRUE) // If they're dead, force their bloodstream to update briefly.
				updatehealth()
				if (stat != DEAD)
					to_chat(src, "<span class='notice'>You feel a breath of fresh air enter your lungs. It feels good.</span>")

			// VOREStation Edit - Begin
			else if (istype(H) && attempt_to_scoop(H))
				return FALSE;
			// VOREStation Edit - End
