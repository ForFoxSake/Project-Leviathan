...
/datum/reagent/proc/on_mob_life(var/mob/living/carbon/M, var/alien, var/datum/reagents/metabolism/location, var/cpr_push = 0) // Currently, on_mob_life is called on carbons. Any interaction with non-carbon mobs (lube) will need to be done in touch_mob.
	if(!istype(M))
		return
	if(!cpr_push && !affects_dead && M.stat == DEAD) // Ignore dead-ness when recieving cpr.
		return
	if(!istype(location))
		return
