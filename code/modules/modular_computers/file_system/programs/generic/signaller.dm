...
/datum/computer_file/program/signaller/is_supported_by_hardware(hardware_flag, loud, mob/user)
	if(!..())
		return FALSE

	if(!computer?.network_card.check_functionality() || computer.network_card.ethernet)
		if(loud)
...
/datum/computer_file/program/signaller/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = default_state)
	var/list/data = computer.get_header_data()
	data["freq"] = computer?.network_card.radio_connection.frequency
	data["code"] = code
...
/datum/computer_file/program/signaller/Topic(href, href_list)
	...
	if(href_list["change_freq"])
		computer.network_card.set_frequency(computer.network_card.radio_connection.frequency + text2num(href_list["change_freq"]))
		return 1

	if(href_list["edit_freq"])
		var/input_freq = input("Enter signal frequency ([RADIO_LOW_FREQ]-[RADIO_HIGH_FREQ]):", "Signal parameters", computer.network_card.radio_connection.frequency) as num|null
	...
	if(href_list["save_signal"])
		if(length(saved_signals) >= max_saved)
			return 1
		saved_signals.Add(list(list("freq"=computer.network_card.radio_connection.frequency, "code"=code)))
		return 1
