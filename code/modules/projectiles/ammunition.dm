...
/obj/item/ammo_magazine/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen))
		var/tmp_label = ""
		var/label_text = sanitizeSafe(input(user, "Write some text into \the [initial(name)]","Label",tmp_label), MAX_NAME_LEN)
		if(length(label_text) > 20)
			to_chat(user, "<span class='warning'>The label can be at most 20 characters long.</span>")
		else if(!label_text)
			to_chat(user, "<span class='notice'>You scribble the label off of [initial(src)].</span>")
			name = initial(name)
		else
			to_chat(user, "<span class='notice'>You inscribe \"[label_text]\" into \the [initial(name)].</span>")
