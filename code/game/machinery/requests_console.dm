...
/obj/machinery/requests_console/attackby(var/obj/item/weapon/O as obj, var/mob/user as mob)
	...
	if(I)
		if(inoperable(MAINT)) return
		if(screen == RCS_MESSAUTH)
			msgVerified = text("<font color='green'><b>Verified by [I.registered_name] ([I.assignment])</b></font>")
			updateUsrDialog()
		if(screen == RCS_ANNOUNCE)
			if(access_RC_announce in I.GetAccess())
				announceAuth = 1
				announcement.announcer = I.assignment ? "[I.assignment] [I.registered_name]" : I.registered_name
			else
				reset_message()
