/obj/machinery/computer/shuttle
	...
	attackby(var/obj/item/weapon/card/W as obj, var/mob/user as mob)
		...
		if (I)
			if (!I.access) //no access
				to_chat(user, "The access level of [I.registered_name]\'s card is not high enough. ")
				return

			var/list/cardaccess = I.access
			if(!istype(cardaccess, /list) || !cardaccess.len) //no access
				to_chat(user, "The access level of [I.registered_name]\'s card is not high enough. ")
				return

			if(!(access_heads in I.access)) //doesn't have this access
				to_chat(user, "The access level of [I.registered_name]\'s card is not high enough. ")
				return 0
			...
			switch(choice)
				if("Authorize")
					src.authorized -= I.registered_name
					src.authorized += I.registered_name
