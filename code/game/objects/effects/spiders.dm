...
/obj/effect/spider/spiderling
	...
	var/list/grow_as = list(/mob/living/simple_mob/animal/giant_spider/tunneler, /mob/living/simple_mob/animal/giant_spider/nurse, /mob/living/simple_mob/animal/giant_spider/hunter, /mob/living/simple_mob/animal/giant_spider/webslinger, /mob/living/simple_mob/animal/giant_spider/lurker)
...
/obj/effect/spider/spiderling/New(var/location, var/atom/parent)
	...
	pixel_y = rand(6,-6)
	START_PROCESSING(SSobj, src)
/*	// Leviathan Edit: We've got a subclass for non-growing ones, lets not nerf these things even harder.
	//50% chance to not grow up
	if(prob(50))
		amount_grown = -1
*/
	get_light_and_color(parent)
...
/obj/effect/spider/spiderling/process()
	if(travelling_in_vent)
		if(istype(src.loc, /turf))
			...
			return // In a vent and waiting on a timer. Just chill for now.
	else if(entry_vent)
		if(loc == entry_vent) // Booyah we're in the vents!
			...
			if(entry_vent.network && entry_vent.network.normal_members.len)
				...
				travelling_in_vent = TRUE // Don't want to pick multiple locations.
				var/travel_time = round(get_dist(loc, exit_vent.loc) / 2)
				spawn(travel_time)
					if(prob(50))
						var/obj/machinery/atmospherics/unary/vent_pump/loud_vent = exit_vent
						if(prob(50))
							loud_vent = entry_vent
						loud_vent.visible_message("<span class='notice'>You hear something squeezing through the ventilation ducts.</span>",2)

					spawn(travel_time)
						if(!exit_vent || exit_vent.welded)
							travelling_in_vent = FALSE  // No exit here, we'll get 'em next time, boss!
							return
			...
			else // We have nowhere to go, abandon our hidey hole!
				loc = get_turf(entry_vent.loc)
				if(entry_vent.welded) // Whoops, we got welded in with nowhere to go. Showing a special message here so no spiderling gets left behind.
					src.visible_message("<span class='notice'>[src] tumbles out of a crack in the welded vent!</span>",2)
				entry_vent = null
		else if(get_dist(src, entry_vent) <= 1 && !entry_vent.welded) // Not in a vent yet, but we could be!
			if(prob(50))
				src.visible_message("<B>[src] scrambles into the ventillation ducts!</B>")
			loc = entry_vent
			return
		else // We're not in the vent and we're not near the vent. We'll just give up on that vent.
			entry_vent = null
	//=================
