...
/obj/item/device/communicator
	...
	var/obj/item/weapon/card/id/id = null //Making it possible to slot an ID card into the communicator so it can function as both.
	var/owner = ""
	var/occupation = ""
	var/alert_called = 0
...
/obj/item/device/communicator/proc/register_to_holder()
	...
	if(ismob(loc))
		...
		if(ismob(S.loc))
			locmob = S.loc
	if(locmob)
		register_device(locmob.name)
		if(locmob.mind) // Messy, but easier than finding a borg's id.
			if(locmob.mind.role_alt_title)
				occupation = locmob.mind.role_alt_title
			else if(locmob.mind.assigned_role)
				occupation = locmob.mind.assigned_role
		initialize_exonet(locmob)
...
/obj/item/device/communicator/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/card/id))
		...
		if(!idcard.registered_name || !idcard.assignment)
			...
			if(occupation != idcard.assignment)
				...
				if(!id)
					...
					id = idcard
					to_chat(usr, "<span class='notice'>You slot \the [idcard] into \the [src].</span>")
				else
					to_chat(user, "<span class='notice'>The ID slot is already full.</span>")
...
// Proc: GetAccess()
// Description: Same things PDAs do for id access.
...
/obj/item/device/communicator/proc/remove_pen()
	var/obj/item/weapon/pen/O = locate() in src
	if(O)
		if(istype(loc, /mob))
			var/mob/M = loc
...
/obj/item/device/communicator/CtrlClick()
	if(issilicon(usr))
		return
