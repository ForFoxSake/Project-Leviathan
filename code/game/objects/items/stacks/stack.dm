...
/obj/item/stack/proc/list_recipes(mob/user as mob, recipes_sublist)
	...
	var/dat = "<TT>Amount Left: [src.get_amount()]<br>"
	for(var/i=1;i<=recipe_list.len,i++)
		...
		if (istype(E, /datum/stack_recipe_list))
			...
			dat += "<a href='?src=\ref[src];sublist=[i]'>[srl.title]</a>"
		...
		if (istype(E, /datum/stack_recipe))
			...
			if (can_build)
				dat += text("<A href='?src=\ref[src];sublist=[recipes_sublist];make=[i];multiplier=1'>[title]</A>  ")
			else
				dat += text("[]", title)
				continue
			if (R.max_res_amount>1 && max_multiplier>1)
				...
				for (var/n in multipliers)
					if (max_multiplier>=n)
						dat += " <A href='?src=\ref[src];make=[i];multiplier=[n]'>[n*R.res_amount]x</A>"
				if (!(max_multiplier in multipliers))
					dat += " <A href='?src=\ref[src];make=[i];multiplier=[max_multiplier]'>[max_multiplier*R.res_amount]x</A>"

	dat += "</TT>"
	var/datum/browser/popup = new(user, "stack", "Constructions from [src]", 350, 600, src)
